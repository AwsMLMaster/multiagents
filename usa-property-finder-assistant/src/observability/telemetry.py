"""
Observability Layer for USA Property Finder Assistant.

Provides:
- OpenTelemetry distributed tracing
- LangSmith LLM tracing
- Metrics collection (latency, cache hit rate, search result counts)
- Structured logging
"""
...
class MetricsCollector:
    ...
    def record_latency(self, name: str, latency_ms: float, tags: Optional[Dict] = None):
        key = self._make_key(name, tags)
        self._histograms.setdefault(key, []).append(latency_ms)
    ...
    def _publish_cloudwatch_metric(self, name: str, value: float, unit: str, tags: Optional[Dict] = None):
        ...
        try:
            ...
            if tags:
                dimensions.extend([{"Name": k, "Value": str(v)} for k, v in tags.items()])

            self.cloudwatch.put_metric_data(
                Namespace="USAPropertyFinderAssistant",
...
class Telemetry:
    ...
    @contextmanager
    def trace_request(self, request_id: str, session_id: Optional[str] = None):
        """Trace an incoming request."""
        start_time = time.time()
        attributes = {"request.id": request_id, "session.id": session_id or "none"}
    ...
    def trace_agent(self, agent_name: str):
        ...
        with self.otel.start_span(f"agent.{agent_name}", {"agent.name": agent_name}) as span:
            self.metrics.increment("agent_executions_total", tags={"agent": agent_name})
            try:
                yield span
                latency_ms = (time.time() - start_time) * 1000
                self.metrics.record_latency("agent_latency", latency_ms, tags={"agent": agent_name})
            except Exception as e:
                self.metrics.increment("agent_executions_error", tags={"agent": agent_name})
                self.otel.record_exception(e, span)
                raise

    @contextmanager
    def trace_tool(self, tool_name: str):
        """Trace a tool/API call execution (e.g., search_listings, geocode_location)."""
        start_time = time.time()

        with self.otel.start_span(f"tool.{tool_name}", {"tool.name": tool_name}) as span:
            try:
                yield span
                latency_ms = (time.time() - start_time) * 1000
                self.metrics.record_latency("tool_latency", latency_ms, tags={"tool": tool_name})
            except Exception as e:
                self.otel.record_exception(e, span)
                raise
